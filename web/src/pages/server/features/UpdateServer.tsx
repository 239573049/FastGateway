...
export default function UpdateServer({
    ...
    const fallbackValue = useMemo<Server>(
        () => ({
            name: "",
            listen: 80,
            description: "",
    ...
    const [value, setValue] = useState<Server>(fallbackValue);

    useEffect(() => {
        if (!visible) return;
        setTab("basic");
        setIsSaving(false);
        setValue(server ? { ...server } : fallbackValue);
    }, [fallbackValue, server, visible]);
    ...
    const save = async () => {
        ...
        setIsSaving(true);
        try {
            await updateServer(value.id, { ...value, name });
            toast.success("更新成功");
            onOk();
        } catch {
            toast.error("更新失败");
                    ...
                    >
                        ...
                        <TabsContent value="basic" className="space-y-4">
                            <div className="grid gap-4 md:grid-cols-2">
                                <div className="space-y-2">
                                    <FieldLabel
                                        htmlFor="update-server-listen"
                                        label="服务端口"
                                        tooltip="设置代理服务监听的端口。"
                                    />
                                    <Input
                                        id="update-server-listen"
                                        type="number"
                                        value={value.listen}
                                        onChange={(e) =>
                                            setValue((prev) => ({
                            ...
                            <div className="space-y-2">
                                <FieldLabel
                                    htmlFor="update-server-desc"
                                    label="服务描述"
                                    tooltip="用于备注用途与上下文，便于团队协作。"
                                />
                                <Textarea
                                    id="update-server-desc"
                        ...
                        <TabsContent value="limits" className="space-y-4">
                            <div className="grid gap-4 md:grid-cols-2">
                                <div className="space-y-2">
                                    <FieldLabel
                                        htmlFor="update-server-max-body"
                                        label="最大请求体大小(字节)"
                                        tooltip="最大请求体大小限制（字节）。留空表示不限制。"
                                    />
                                    <Input
                                        id="update-server-max-body"
                                        type="number"
                                ...
                                <div className="space-y-2">
                                    <FieldLabel
                                        htmlFor="update-server-timeout"
                                        label="超时时间(秒)"
                                        tooltip="请求超时时间（秒）。默认 900 秒（15 分钟）。"
                                    />
                                    <Input
                                        id="update-server-timeout"
                                        type="number"
                                        value={value.timeout}
                                        onChange={(e) =>
                                            setValue((prev) => ({
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                                <div className="space-y-2">
                                    <FieldLabel
                                        htmlFor="update-server-failover-connect-timeout"
                                        label="故障转移连接超时(ms)"
                                        tooltip="仅在启用请求级故障转移后生效，用于快速判定单个上游节点连接失败。"
                                    />
                                    <Input
                                        id="update-server-failover-connect-timeout"
                                        type="number"
                                ...
                                <div className="space-y-2">
                                    <FieldLabel
                                        htmlFor="update-server-failover-budget"
                                        label="故障转移总预算(ms)"
                                        tooltip="单个请求在多个健康节点之间切换的总耗时预算。"
                                    />
                                    <Input
                                        id="update-server-failover-budget"
                                        type="number"
                        ...
                        <TabsContent value="features" className="space-y-4">
                            <div className="space-y-2 rounded-lg border bg-muted/30 p-3">
                                <FieldLabel
                                    htmlFor="update-server-client-ip-source"
                                    label="客户端 IP 获取方式"
                                ...
                                >
                                    <SelectTrigger id="update-server-client-ip-source">
                            ...
                            <div className="grid gap-3 md:grid-cols-2">
                                <SettingSwitch
                                    id="update-server-enable"
                                    label="启动服务"
                                    description="是否启用当前服务，或跟随网关启动。"
                                ...
                                />
                                <SettingSwitch
                                    id="update-server-static-compress"
                                    label="静态压缩"
                                ...
                                />
                                <SettingSwitch
                                    id="update-server-request-failover"
                                    label="请求级故障转移"
