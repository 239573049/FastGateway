...
namespace Certes.Acme
{
    ...
    public class AcmeHttpClient : IAcmeHttpClient
    {
        ...
        {
            new ProductInfoHeaderValue("Certes", "3.0.4"),
        ...
        {
            var payloadJson = JsonSerializer.Serialize((JwsPayload)payload, CertesJsonContext.Default.JwsPayload);
        ...
        private async Task<AcmeHttpResponse<T>> ProcessResponse<T>(HttpResponseMessage response, Uri requestedUri)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                if (IsJsonMedia(response.Content?.Headers.ContentType?.MediaType))
                {
                    ...
                    resource = (T)JsonSerializer.Deserialize(json, CertesJsonContext.Default.GetTypeInfo(typeof(T)));
                }
            ...
            else
            {
                ...
                {
                    var json = await response.Content.ReadAsStringAsync();
                    error = JsonSerializer.Deserialize(json, CertesJsonContext.Default.AcmeError);
                }
                else
                {
