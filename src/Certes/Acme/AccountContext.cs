...
namespace Certes.Acme
{
    ...
    internal class AccountContext : EntityContext<Account>, IAccountContext
    {
            ...
            string eabKeyId = null, string eabKey = null, string eabKeyAlg = null)
        {
            ...
            {
                var alg = eabKeyAlg?.ToUpper() ?? "HS256";
                var header = new ExternalAccountBindingHeader
                {
                    Alg = alg,
                    Kid = eabKeyId,
                    Url = endpoint,
                ...
                var headerJson = JsonSerializer.Serialize(header, CertesJsonContext.Default.ExternalAccountBindingHeader);
                var protectedHeaderBase64 = JwsConvert.ToBase64String(Encoding.UTF8.GetBytes(headerJson));

                var jwk = context.AccountKey.JsonWebKey;
                var accountKeyBase64 = JwsConvert.ToBase64String(
                    Encoding.UTF8.GetBytes(
                        JsonSerializer.Serialize(jwk, CertesJsonContext.Default.GetTypeInfo(jwk.GetType()))));

                var signingBytes = Encoding.ASCII.GetBytes($"{protectedHeaderBase64}.{accountKeyBase64}");
                ...
                body.ExternalAccountBinding = new ExternalAccountBinding
                {
