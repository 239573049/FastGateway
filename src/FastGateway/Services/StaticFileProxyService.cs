...
public class StaticFileProxyService
{
    ...
    {
        var staticFileProxyEntities = await FreeSqlContext.FreeSql.Select<StaticFileProxyEntity>().ToListAsync();
    ...
    public async Task<ResultDto> CreateAsync(StaticFileProxyEntity entity)
    {
        ...
        {
            var entities = await FreeSqlContext.FreeSql.Select<StaticFileProxyEntity>()
        ...
        {
            if (await FreeSqlContext.FreeSql.Select<StaticFileProxyEntity>().AnyAsync(x => x.Path == entity.Path))
    ...
    {
        var value = await FreeSqlContext.FreeSql.Select<StaticFileProxyEntity>().Where(x => x.Id == entity.Id).FirstAsync();

        if (value == null) return ResultDto.Error("不存在");
        ...
        {
            if (await FreeSqlContext.FreeSql.Select<StaticFileProxyEntity>().AnyAsync(x => x.Id != entity.Id && x.Path == entity.Path))
                return ResultDto.Error("已存在相同的路径");
        }
        ...
        await FreeSqlContext.FreeSql.Update<StaticFileProxyEntity>().SetSource(value).ExecuteAffrowsAsync();
    ...
    {
        if (!await FreeSqlContext.FreeSql.Select<StaticFileProxyEntity>().AnyAsync(x => x.Id == id)) return ResultDto.Error("不存在");

        await FreeSqlContext.FreeSql.Delete<StaticFileProxyEntity>().Where(x => x.Id == id).ExecuteAffrowsAsync();
    ...
    {
        var query = FreeSqlContext.FreeSql.Select<StaticFileProxyEntity>()
