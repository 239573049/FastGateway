...
public class LinuxSystemUsage : ISystemUsage
{
    ...
    private long previousSectorsRead;
    private long previousSectorsWritten;
    ...
    public (float memoryUsage, ulong totalMemory, ulong useMemory) GetMemoryUsage()
    {
        ...
        var memoryUsage = (float)usedMemory / totalMemory * 100;
    ...
    public (float read, float write) GetDiskUsage()
    {
        ...
        foreach (var line in lines)
        {
            ...
            if (parts.Length > 13)
            {
                var deviceName = parts[2]; // 设备名，通常是第三列
                var readsCompleted = long.Parse(parts[3]); // 完成的读操作
                var sectorsRead = long.Parse(parts[5]); // 读取的扇区数
                var writesCompleted = long.Parse(parts[7]); // 完成的写操作
                var sectorsWritten = long.Parse(parts[9]); // 写入的扇区数
                var byteRead = (sectorsRead - previousSectorsRead) * 512; // 每个扇区512字节
                var byteWritten = (sectorsWritten - previousSectorsWritten) * 512;
